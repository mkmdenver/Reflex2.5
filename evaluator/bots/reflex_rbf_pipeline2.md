...
* `tier_requests.listen.start channel=reflex:liveA:cmd.tiers ...`
...
* `stage2.raise_tier.requested ... channel: reflex:liveA:cmd.tiers`
* Active set key example: `reflex:liveA:LIVE:run0:eval:fts_simple2:active`
...
* In Evaluator logs:
  ...
  * Is instance `liveA` or `default`?
  * Is raise channel `reflex:liveA:cmd.tiers` or `eval.raisetier`?
  * Is `pg_dsn_set=true`?
  * Is universe non-empty?

* In DataHub logs:

  * Do you see `tier_requests.listen.start channel=reflex:liveA:cmd.tiers`?

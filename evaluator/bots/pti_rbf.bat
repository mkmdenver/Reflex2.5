...
set "PTI_FILTER_STREAM_CHANNEL=eval.rbf_filter_stream"
...
if "%PTI_SYMBOLS_MODE%"=="static" (
  ...
  set "PTI_ACTIVE_SET_KEY=eval:fts_rbf:active"
  set "PTI_SYMBOLS="  
)

REM Where intents are recorded (record/tee)
set "PTI_INTENT_RECORD_PATH=%ROOT%evaluator\runs\pti_rbf\intents.jsonl"
...
echo [RUNNING] %ROOT%evaluator\bots\PTI_rbf.py

    ...
    try:
        async for msg in ps.listen():  # type: ignore[attr-defined]
            ...
            if ok and sym not in active:
                ...
                # Minimum hold time (prevents add/remove flapping)
                since = active_since.get(sym, now_sec)
                if (now_sec - since) < cfg.min_active_hold_s:
                    continue

                # Exit hysteresis: don't drop just because pct_up dips a bit
                if pct >= cfg.pct_up_exit:
                    continue

                # Only remove when the Stage2 gate actually fails
                if ok:
                    continue

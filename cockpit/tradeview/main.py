...
tradeview_PORT = int(_env("tradeview_PORT", "7011"))
...
assets_dir = os.path.join(ROOT, "cockpit", "tradeview", "templates", "dist", "assets")
...
async def index() -> str:
    p = os.path.join(ROOT, "cockpit", "tradeview", "templates", "dist", "index.html")
...
async def time_sync() -> Dict[str, Any]:
    # Keep it simple: server time (tradeview host) is enough for UI offset.
    utc_ms = int(time.time() * 1000)
    return {"server_utc_ms": utc_ms, "server_iso": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "source": "tradeview"}
...
async def plans_active(account_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    trader_ok = False
    note = "Plans not implemented in Trader yet (stub response from tradeview)."

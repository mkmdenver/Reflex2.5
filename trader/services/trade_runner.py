...
class TradeRunner:
    ...
    async def _read_md(self, symbol: str) -> Tuple[Optional[float], Optional[float], Optional[float]]:
        # Delegate to md_worker if store has it; otherwise no-op.
        try:
            md = self.store.get_md(symbol) or {}
            lt = _as_float(md.get("lt"))
    ...
    async def _step_trade(self, t: Dict[str, Any]) -> None:
        ...
        if st in ("NEW", "ADOPTED"):
            # Adopted positions: we already have a broker position; treat as active.
            t["state"] = "ACTIVE"
            self._persist(
                t,
                "TRADE_ACTIVE",
        ...
        if st == "ENTRY_PENDING":
            # Determine session and entry type
            sess = str(t.get("market_session") or _session_kind_now())
            extended = sess in ("PRE", "POST")
            ...
            try:
                # Extended hours: must use limit orders (market not allowed)
                if extended:
            ...
            # Optional broker-side protection (real STOP order). Soft exits remain the smart layer.
            await self._ensure_broker_protection(aid, adapter, t)
                        return
        ...
        if st == "ENTRY_SUBMITTED":
            ...
            try:
                t["entry_fill_latency_ms"] = int((float(t.get("entry_filled_ts") or _now()) - float(t.get("entry_submitted_ts") or _now())) * 1000.0)
            except Exception:
                t["entry_fill_latency_ms"] = None

            # Compute stop + target now (simple rule)
            ...
            # For now, set stop/target same as soft
            t["stop_price"] = t["soft_stop_price"]
            t["take_profit_price"] = t["soft_target_price"]

            # Auto tier raise so we receive ticks/quotes for this symbol
            await self._raise_symbol_tier(sym)
        ...
        if st in ("ACTIVE", "EXIT_SUBMITTED"):
            # If position is already closed at broker, mark done.
            snap = self._snapshot_for_account(aid)
            pos = self._find_pos(snap, sym)
            ...
            # --- broker position qty snapshot (for reduce-only exits) ---
            broker_qty = float(_as_float(pos.get("qty")) or 0.0)

            # Guard: if we ever find ourselves short in a long trade (or vice-versa),
            # do NOT allow an exit that could increase exposure. Attempt a safe cover/close.
            ...
            lt, bid, ask = await self._read_md(sym)
            md_ok = not (lt is None and bid is None and ask is None)

            last_px = lt if lt is not None else (bid if bid is not None else ask)
            if last_px is None:
                # Still advance + emit metrics using fallback price.
                px_fallback = (
            ...
            # Always update/emit metrics (even if we don't exit this cycle).
            ...
            # Choose exit model
            exit_model = str(t.get("profit_model") or self.exit_model_default).lower()
            if exit_model == "tslfe" and self.tslfe_enabled:
                # For now we still use stop/target as triggers, but TSLFE metrics guide chase.
                pass

            if t.get("state") == "ACTIVE":
                if hit_stop:
                    await self._submit_soft_exit(
                        aid, sym, adapter, t, kind, "stop", lt, bid, ask, reason_hint=t.get("exit_reason_hint"), current_pos_qty=broker_qty
                    )
                    return
                if hit_target:
                    await self._submit_soft_exit(
                        aid, sym, adapter, t, kind, "target", lt, bid, ask, reason_hint="HARD_TARGET", current_pos_qty=broker_qty
                    )
                    return

            # If we already have an exit working, chase it if needed.
            await self._chase_exit_if_needed(aid, sym, adapter, t, kind, lt, bid, ask, current_pos_qty=broker_qty)

            # Auto tier raise to keep MD flowing
            await self._raise_symbol_tier(sym)
            return

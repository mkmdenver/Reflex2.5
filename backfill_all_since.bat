...
REM
REM Assumes:
REM   - .env and requirements.txt live in project root
REM   - db_backfill module at tools\symbol_manager\db_backfill.py
REM   - Creates / uses .venv in project root
REM ============================================================
...
REM ------------------------------------------------------------
REM Run backfill module for ALL symbols: daily, minute, tick
REM ------------------------------------------------------------
set "PY=%PYTHON_EXE%"
...
if errorlevel 1 (
    echo [ERROR] DAILY backfill failed with exit code %errorlevel%.
    exit /b %errorlevel%
)
...
if errorlevel 1 (
    echo [ERROR] MINUTE backfill failed with exit code %errorlevel%.
    exit /b %errorlevel%
)
...
echo.
echo [OK] Backfill complete for ALL (daily + minute + tick) since %SINCE%
endlocal
exit /b 0

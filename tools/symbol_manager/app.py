...
# ---------------------------------------------------------------------------
# Purpose:
#   • Immediate local echo (show order the moment user clicks)
#   • Merge broker truth back into local state (reconcile on read)
#
# Intentionally simple: in-memory only for now.
# ---------------------------------------------------------------------------

_LOCAL_ORDERS: Dict[str, Dict[str, Any]] = {}     # key = order id (client id initially, may become broker id)
...
def _upsert_local_from_broker(aid: str, broker_doc: Dict[str, Any]) -> None:
    ...
    # Find any matching client id mapping
    client_id = None
...
async def api_list_orders(
    ...
    Returns local ledger (immediate echo) + broker truth (merged into local on read).
    """
    if status not in ("active", "closed"):
        status = "active"

    # 1) Pull broker orders (truth) and merge into local
    if account_id:
        ods = await _list_orders_for_account(account_id, status=status)
        for od in ods:
    ...
    # Stable-ish ordering: newest first by submitted_at (string ISO sorts OK if consistent)
    out.sort(key=lambda x: str(x.get("submitted_at") or ""), reverse=True)
    return {"ok": True, "orders": out}

def _fail_local(client_id: str, aid: str, reason: str) -> None:
    o = _LOCAL_ORDERS.get(client_id)
    if not o:
        return
    o["status"] = "failed"
    o["error"] = reason
    publish_event({"type": "ORDER_LOCAL_FAILED", "ts": time.time(), "account_id": aid, "order": _local_order_doc(o), "error": reason})
...
async def api_orders_place(request: Request):
    ...
    #Normalize once, overwrite once
    qty_f = _as_float(data.get("qty"))
    if not qty_f or qty_f <= 0:
        return JSONResponse(...)
    ...
    if args["side"] == "sell" and not allow_short:
        ...
        if req_qty > pos_qty + 1e-9:
            _fail_local(client_id, aid, reason=f"sell qty {req_qty:g} exceeds position qty {pos_qty:g} (short not allowed)")
            return JSONResponse(
                {"ok": False, "error": f"sell qty {req_qty:g} exceeds position qty {pos_qty:g} (short not allowed)"},
                status_code=400,
            )
    ...
    if not args["symbol"] or not args["side"] or not args["qty"]:
        ...
        return JSONResponse(
            {"ok": False, "error": "symbol, side, qty required"},
            status_code=400,
        )
    ...
    log.info(
    "orders.final_broker_payload",
    extra={"account_id": aid, "payload": broker_args}
    )
    ...
    if pm_fn is not None:
        try:
            ...
            # ------------------------------------------------------------------
            # Reconcile local echo -> broker response (id/status)
            # ------------------------------------------------------------------
            try:
                broker_id = None
                if isinstance(res, dict):
                    broker_id = res.get("id") or res.get("order_id") or res.get("client_order_id")
                if broker_id:
                    _CLIENT_TO_BROKER[client_id] = str(broker_id)
                    local0 = _LOCAL_ORDERS.pop(client_id, None) or {}
                    local0.update({
                        "account_id": aid,
                        "id": str(broker_id),
                        "status": (res.get("status") if isinstance(res, dict) else None) or "submitted",
                    })
                    _LOCAL_ORDERS[str(broker_id)] = local0
                else:
                    if client_id in _LOCAL_ORDERS:
                        _LOCAL_ORDERS[client_id]["status"] = "submitted"
            except Exception:
                pass

            publish_event({"type": "ORDER_SUBMIT_OK", "ts": time.time(), "account_id": aid, "result": res})
        ...
        try:
            if client_id in _LOCAL_ORDERS:
                _LOCAL_ORDERS[client_id]["status"] = "failed"
        except Exception:
            pass
        publish_event({"type": "ORDER_SUBMIT_FAILED", "ts": time.time(), "account_id": aid, "error": str(e)})
        return JSONResponse({"ok": False, "error": str(e)}, status_code=400)
...
async def api_list_trades(status: str = Query("active", alias="status"), account_id: Optional[str] = Query(None, alias="account_id")):
    ...
    status: active|closed (active = ENTERING/OPEN/EXITING, closed = DONE/CANCELED/ERROR)
    """
    if status not in ("active", "closed"):
        status = "active"
    ...
    for t in _MANAGED_TRADES.values():
        if account_id and t.get("account_id") != account_id:
            continue
        st = t.get("state")
        if status == "active" and st in active_states:
            out.append(t)

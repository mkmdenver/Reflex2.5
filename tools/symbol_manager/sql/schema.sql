...
FROM minute_bars;
-- === Tick cache (DB) ===
CREATE TABLE IF NOT EXISTS reflex.trade_ticks (
  symbol     TEXT NOT NULL,
  timestamp  TIMESTAMPTZ NOT NULL,
...
-- If Timescale is present:
SELECT create_hypertable('reflex.trade_ticks','timestamp',
                         chunk_time_interval => INTERVAL '1 day',
                         if_not_exists => TRUE);
CREATE INDEX IF NOT EXISTS ix_trade_ticks_sym_time ON reflex.trade_ticks (symbol, timestamp DESC);
CREATE INDEX IF NOT EXISTS ix_trade_ticks_time     ON reflex.trade_ticks (timestamp DESC);
-- Optional rolling retention
SELECT add_retention_policy('reflex.trade_ticks', INTERVAL '10 days');

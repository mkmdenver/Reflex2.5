...
async def main() -> int:
    ...
    ap.add_argument("--instance", default=None, help="Instance id for scoping clock channel (default: REFLEX_INSTANCE_ID).")
    ...
    ap.add_argument("--clock-every", type=int, default=1, help="Publish clock pulse every N emitted events (default 1).")
    ...
    clock_ch = _resolve_clock_channel(instance_id, args.clock_channel)
    ...
    print(f"[INFO] clock_channel={clock_ch} (instance={instance_id})")
    ...
    for row in _iter_parquet_rows(path):
        ...
        # Publish replay clock pulse based on the event timestamp (engine time).
        # Trader can consume this to gate at_time triggers deterministically.
        et = _coerce_epoch_seconds(payload_for_ts.get("t")) if payload_for_ts else None
        if et is not None:
            emitted_events += 1
            every = int(args.clock_every or 1)
            if every <= 1 or (emitted_events % every) == 0:
                await pub.publish(clock_ch, pack({"t": float(et)}))
                clock_pulses += 1
    ...
    print(f"[DONE] rows_seen={seen} sent_ticks={sent_ticks} sent_quotes={sent_quotes} clock_pulses={clock_pulses}")
    return 0
